...
async function buildTrace(deps) {
  ...
  return {
    $comment: "GENERATED by scripts/generate-gateway-trace.mjs from the real Agent Trust Gateway. Do not edit by hand.",
    wire: "aimtp/0.1",
...
async function main() {
  ...
  if (CHECK) {
    ...
    if (readFileSync(OUT, "utf8") !== serialised) {
      throw new Error("data/gateway-trace.json is stale — the Gateway's behaviour changed. Run `npm run trace`.");
    }
    console.log(`trace:check ok — ${trace.scenarios.length} scenarios, ${trace.audit.length} audit events`);
    return;
  }
